...
namespace HumanResourceHelth.Web.Areas.Admin.Controllers
{
    public class PracticeQuestionsController : Controller
    {
        ...
        public ActionResult Details(int? id)
        {
            ...
            PracticeQuestions practiceQuestions = db.PracticeQuestions.Find(id);
        ...
        public ActionResult Create([Bind(Include = "Id,Question,QuestionAr,Practice,Status")] PracticeQuestions practiceQuestions)
        {
            if (Session["UserId"] == null) return Redirect(Url.Action("Index", "Login", new { Area = "Admin", r = Request.Url.ToString() }));
            if (ModelState.IsValid)
            {
                practiceQuestions.Respondent = int.Parse(Request["Respondent"].ToString());
                db.PracticeQuestions.Add(practiceQuestions);
                db.SaveChanges();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Id,Question,QuestionAr,Practice,Status")] PracticeQuestions practiceQuestions)
        {
            if (Session["UserId"] == null) return Redirect(Url.Action("Index", "Login", new { Area = "Admin", r = Request.Url.ToString() }));
            if (ModelState.IsValid)
            {
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            if (Session["UserId"] == null) return Redirect(Url.Action("Index", "Login", new { Area = "Admin", r = Request.Url.ToString() }));
            PracticeQuestions practiceQuestions = db.PracticeQuestions.Find(id);
            db.PracticeQuestions.Remove(practiceQuestions);
            db.SaveChanges();
            return RedirectToAction("Index");
        }
        ...
        public ActionResult CheckPlan()
        {
            ...
            int functionId = db.FunctionPractices.Find(practiceId).FunctionId;
            int PlanId = db.Functions.Find(functionId).planId;
        ...
        {
            int practiceId = Int16.Parse(Request["practiceId"].ToString());
            var practiceQuestions = db.PracticeQuestions.Where(x => x.Practice == practiceId).ToList();
            //Dictionary<string, dynamic> response = new Dictionary<string, dynamic>();
